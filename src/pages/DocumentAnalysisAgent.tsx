...
const analysisStages = [
  ...
  "Считаем потребность по месяцам",
  "Считаем ожидаемые поступления из графика отгрузок",
  "Считаем контрольные точки логистики",
  "Считаем прогнозируемый остаток по месяцам",
  "Формируем result.xlsx"
] as const;
...
const requiredFileRoles = [
  {
    ...
    hint: "помесячный план изделий",
    required: true
  },
  {
    role: "detailed_production_schedule",
    label: "Детальный график производства",
    hint: "по дням / неделям · только определение роли",
    required: false
  },
  {
    role: "stock",
